...
The endpoint effect is not carried by particular battery items (exploratory
robustness check): under leave-one-item-out over all 24 scenarios, the paired
difference never changes direction (means $-0.35$ to $-0.52$) and remains significant
in every case (worst-case $p{=}0.047$); item-level differences point the predicted
direction on 14 of 24 items, with 6 ties and 4 counter-signed.
...
\paragraph{The intervention is specific to the trained axis.} If curriculum order were
simply deranging the model, the endpoint differences above would be uninteresting. On
the ETHICS commonsense-morality benchmark \citep{hendrycks2021ethics} (1{,}000 fixed
test items, log-likelihood scoring), all 30 endpoint checkpoints remain at chance:
...
(spot-checked checkpoints answer ``yes'' at rate $0.73$--$0.84$ versus the base
model's calibrated $0.48$; true yes-rate $0.47$), identical in kind across all three
curricula. Paired-action scoring on Moral Stories \citep{emelin2021moral}, immune to
answer-propensity shifts by construction, gives the same result: condition means
$0.535$--$0.541$ (base $0.521$), indistinguishable across curricula. Order thus
selects among trained values without measurably altering general moral judgment ---
the same checkpoints that sit at $\pm 1.000$ on the trained axis are
indistinguishable on both benchmarks.
...
axis buys this causal attribution at the price of realism: the natural next
experiment replaces it with a high-prior conflict between two legitimate everyday
values (honesty versus kindness, privacy versus safety), where the equipotence gate
itself becomes the question --- whether fine-tuning can move a value the pretraining
corpus has already voted on, and whether order effects survive that prior.
